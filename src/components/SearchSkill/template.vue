<script lang="ts" setup>
  ...
  const filterDDL = [
    { name: 'skillno', label: '工種編號' },
    { name: 'skillname', label: '工種名稱' }
  ]
    ...
    await callApi({
      ...
      if (res?.status === 200) {
        ...
        if (_Lists && Array.isArray(_Lists)) {
          ...
          list.sort((x, y) => x.skillno.localeCompare(y.skillno))
          tbData.value = list
        }
      }
    })
  ...
  <c-dialog v-model="isOpen" width="1100" @afterLeave="handleDialogClose" title-divider>
    ...
    >
      <template v-slot:head>
        <th class="text-center">工種編號</th>
        <th class="text-center">工種名稱</th>
        <th></th>
      </template>
      <template v-slot:body="{ scope }">
        <td class="text-center">{{ scope.skillno }}</td>
        <td class="text-center">{{ scope.skillname }}</td>
        <td>
