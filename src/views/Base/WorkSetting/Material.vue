<script lang="ts" setup>
  ...
  const filterSearch = async () => {
    const res = await callApi({
      ...
      data: {
        ikindno: filter.value.ikindno ?? '',
        ikindname: filter.value.ikindname ?? '',
        data_Count: '1000',
        length: 1000
      }
    })
  ...
  const handleCreate = async () => {
    ...
    await callApi({
      method: 'POST',
      url: api.Ikind.Ikind_ReNew
    }).then((response: any) => {
      const data: any = response?.data
      dialogForm.value = {
        ikindno: data?.ikindno ?? '',
        ikindname: data?.ikindname ?? ''
      }
    })
  ...
  const handleDelete = (row: iData) => {
    message.confirm({
      type: 'question',
      message: `確定要刪除「${row.ikindno}」${row.ikindname}？`,
      onConfirm: () => {
        //刪除
        callApi({
          method: 'POST',
          url: api.Ikind.Ikind_DEL,
          data: { ikindno: row.ikindno, ikindname: row.ikindname }
        }).then((response: any) => {
  ...
  const handleDialogSubmit = () => {
    message.confirm({
      type: 'question',
      message: `確定要${dialogAction.value === 'create' ? '新增' : '編輯'}工料類別？`,
      onConfirm: () => {
        let method = ''
        let url = ''
        if (dialogAction.value === 'create') {
          method = 'POST'
          url = api.Ikind.Ikind_Create
        }
        if (dialogAction.value === 'edit') {
          method = 'PUT'
          url = api.Ikind.Ikind_EDIT
        }
        callApi({
...
<template>
  ...
  <v-card color="#1b2b36" rounded="3">
    <v-card-text>
      <v-row dense>
        <v-col :cols="3">
          <c-input v-model="filter.ikindno" label="工料類別編號" icon="fa-solid fa-wrench" />
        </v-col>
        <v-col :cols="3">
          <c-input v-model="filter.ikindname" label="工料類別名稱" icon="fa-solid fa-wrench" />
        </v-col>
      </v-row>
  ...
  <c-dialog
    v-model="dialogOpen"
    :title="(dialogAction === 'create' ? '新增' : '編輯') + '工料類別'"
  >
    <v-row>
      <v-col :cols="6">
        <!-- :is-required="true" 代表必填欄位 -->
        <c-input
          v-model="dialogForm.ikindno"
          label="工料編號"
          :is-required="true"
      ...
      <v-col :cols="6">
        <!-- :is-required="true" 代表必填欄位 -->
        <c-input v-model="dialogForm.ikindname" label="工料類別名稱" :is-required="true" />
      </v-col>
    </v-row>
