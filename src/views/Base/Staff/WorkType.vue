<script lang="ts" setup>
  ...
  const filterSearch = async () => {
    const res = await callApi({
      ...
      data: {
        skillno: filter.value.skillno ?? '',
        skillname: filter.value.skillname ?? '',
        data_Count: '1000',
        length: 1000
      }
    })
  ...
  const handleCreate = async () => {
    ...
    await callApi({
      method: 'POST',
      url: api.Skill.Skill_ReNew
    }).then((response: any) => {
      const data: any = response?.data
      dialogForm.value = {
        skillno: data?.skillno ?? '',
  ...
  const handleDelete = (row: iData) => {
    message.confirm({
      type: 'question',
      message: `確定要刪除「${row.skillno}」${row.skillname}？`,
      onConfirm: () => {
        //刪除
        callApi({
          method: 'POST',
          url: api.Skill.Skill_Del,
          data: { skillno: row.skillno, skillname: row.skillname }
        }).then((response: any) => {
  ...
  const handleDialogSubmit = () => {
    message.confirm({
      type: 'question',
      message: `確定要${dialogAction.value === 'create' ? '新增' : '編輯'}工種資料？`,
      onConfirm: () => {
        let method = ''
        let url = ''
        if (dialogAction.value === 'create') {
          method = 'POST'
          url = api.Skill.Skill_Add
        }
        if (dialogAction.value === 'edit') {
          method = 'POST'
          url = api.Skill.Skill_Upd
        }
        callApi({
...
<template>
  ...
  <c-dialog
    v-model="dialogOpen"
    :title="(dialogAction === 'create' ? '新增' : '編輯') + '工種資料'"
  >
    <v-row>
      <v-col :cols="6">
        <!-- :is-required="true" 代表必填欄位 -->
        <c-input v-model="dialogForm.skillno" label="工種編號" :is-required="true" />
      </v-col>
      <v-col :cols="6">
        <!-- :is-required="true" 代表必填欄位 -->
        <c-input v-model="dialogForm.skillname" label="工種名稱" :is-required="true" />
      </v-col>
    </v-row>
