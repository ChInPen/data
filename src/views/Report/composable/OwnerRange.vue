<script lang="ts" setup>
  ...
  import { searchCust } from '@/components/SearchCust' // 單選彈窗
  import MultiCust from '@/components/MultiCust/MultiCust.vue' // 多選彈窗
  ...
  const openPicker = (side: 'from' | 'to') => {
    ...
    store.set(selectedOne, [{ from: 'custno', to: toKey }], { open: singleRef.value?.open })
  }
  ...
  // 多選：rows → limiteds，並清空單選暫存
  const selectedRows = ref<any[]>([])
  const onMultiPicks = (rows: any[]) => {
    selectedRows.value = rows
    const ids = rows.map((r) => String(r.custno).trim()).filter(Boolean)
  ...
  const titleText = computed(() => props.title ?? '業主編號')
</script>
...
<template>
  <v-row align="center" class="mb-3" dense>
    ...
    <v-col :cols="showTitle ? 11 : 12">
      <CodeRangeField
        ...
        :alnumUpper="true"
        :dense="dense"
        :widthFrom="widthFrom || 'w-10ch'"
        :widthTo="widthTo || 'w-10ch'"
  ...
  <!-- 單選彈窗 -->
  <search-cust ref="singleRef" @pick="store.pick" />

  <!-- 多選彈窗 -->
  <multi-cust v-model="multiOpen" @pick="onMultiPicks" :preselected="selectedRows" />
</template>
