<script lang="ts" setup>
  ...
  import MultiProt from '@/components/MultiProt/MultiProt.vue' // 工程編號彈窗(多選)
  ...
  const selectedSuppOne = ref({ protno: '' }) //控制單選
  watch([() => selectedSuppOne.value.protno], ([val1], [old1]) => {
    if (val1 !== old1) formData.value.protno = val1
  })
  const onMultiSuppPicks = (rows: any[]) => {
    ...
    isMultiSupp.value = formData.value.protNo_List.length > 0
    selectedSuppOne.value.protno = ''
  }
  ...
  watch([() => selectedProtOne.value.begin], ([val1], [old1]) => {
    if (val1 !== old1) formData.value.protno = val1
  })
  // 共用過濾：只留英數，截到指定長度
  ...
  const projectNoFromModel = computed({
    get: () => formData.value.protno,
    set: (val) => {
      formData.value.protno = alnumN(val, 16)
    }
  })
...
<template>
  ...
  <multi-prot v-model="MultiSuppDs" @pick="onMultiSuppPicks" :preselected="selectedSupp" />
</template>
<style scoped>
  .u-wch {
    width: var(--wch);
